...
class RenderRequest(BaseModel):
    prompt: str
    userId: str  # Added userId to the request model for proper tracking in Supabase
    ...
    try:
        ...
        for attempt in range(max_retries):
            ...
            if result.returncode == 0:
                ...
                video_url = upload_file_to_supabase(local_video_path, 123)  # For potential future processing
                upload_video_data_to_supabase(video_url, jobs[job_id]["owner"], user_prompt)  # Store metadata in Supabase
...
async def start_render(request: RenderRequest, background_tasks: BackgroundTasks):
    print(f"Received render request: {request}")
